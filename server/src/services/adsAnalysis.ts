...
export interface AdRecommendation {
  ...
  apply:
    ...
    | { type: "BUDGET"; dailyBudget: number };
  metrics: {
    spend: number;
    revenue: number;
    orders: number;
...
export async function analyzeAds(
  ...
  for (const a of adMap.data ?? [])
    adToCampaign.set(String(a.id), String(a.campaign_id));
  ...
  for (const c of campaignData.data ?? []) {
    ...
    const attr = byCampaign.get(String(c.id)) ?? {
      orders: 0,
      revenue: 0,
      highRisk: 0,
    };
    ...
    if (active && spend >= MIN_SPEND && orders === 0) {
      action = "PAUSE";
      severity = "high";
      apply = { type: "STATUS", status: "PAUSED" };
      reasons.push(
        `Spent ${round(spend)} BDT with no orders traced to it in your inbox — review before pausing.`,
      );
      ...
      apply = { type: "STATUS", status: "PAUSED" };
      reasons.push(
        `Losing money: ${roi.toFixed(1)}× ROI (${round(revenue)} back on ${round(spend)} BDT spend).`,
      );
    } else if (
      active &&
      roi != null &&
      ...
      reasons.push(
        `Strong performer: ${roi.toFixed(1)}× ROI on ${orders} orders. Raise daily budget ${round(dailyBudget)} → ${next} BDT.`,
      );
    } else if (
      active &&
      spend >= MIN_SPEND &&
      ...
      apply = { type: "BUDGET", dailyBudget: next };
      reasons.push(
        `Barely breaking even (${roi.toFixed(1)}×). Trim budget ${round(dailyBudget)} → ${next} BDT and watch.`,
      );
    } else if (active && impressions >= 500 && ctr != null && ctr < LOW_CTR) {
      action = "FIX";
      severity = "low";
      reasons.push(
        `Low click-through (${(ctr * 100).toFixed(2)}%) — the creative/audience may not be resonating.`,
      );
    }
    ...
    if (orders >= 2 && highRiskRate != null && highRiskRate > 0.5) {
      reasons.push(
        `${round(highRiskRate * 100)}% of its orders are high COD-risk — returns may erode the ROI shown.`,
      );
  ...
  recommendations.sort(
    (a, b) =>
      rank[a.severity] - rank[b.severity] || b.metrics.spend - a.metrics.spend,
  );
...
function buildSummary(
  ...
  const parts: string[] = [];
  parts.push(
    `Analyzed ${campaignCount} campaign${campaignCount === 1 ? "" : "s"} — ${round(spend)} BDT spend, ${round(revenue)} BDT attributed revenue${roi != null ? ` (${roi.toFixed(1)}× overall ROI)` : ""}.`,
  );
  if (pause) parts.push(`${pause} draining budget — pause suggested.`);
  if (scale)
    parts.push(
      `${scale} clearly profitable — scale ${scale === 1 ? "it" : "them"} up.`,
    );
  ...
  if (!recs.length)
    parts.push("Everything looks healthy — no changes needed right now.");
  return parts.join(" ");
}
